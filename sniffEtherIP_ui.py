...
class session_GUI:
    def __init__(self,packets,flag):
        ...
        if flag==1:
            self.root.title('Ethernet统计')
        else:
            self.root.title('IP统计')
        self.root.geometry('880x400') # 这里的乘号不是 * ，而是小写英文字母 x
        self.frame0= tk.Frame(self.root)
        self.frame0.place(x=10,y=0,width=860,height=390,)
    ...
    def show_details(self,packets):
        ...
        self.table.column('Packets', width=80, minwidth=60, anchor=S)  # 定义列
        self.table.column('Bytes', width=80, minwidth=60, anchor=S)  # 定义列
        self.table.column('PacketA->B', width=100, minwidth=100, anchor=S)  # 定义列
        self.table.column('PacketB->A', width=100, minwidth=100, anchor=S)  # 定义列
        self.table.column('BytesA->B', width=100, minwidth=100, anchor=S)  # 定义列
        self.table.column('BytesB->A', width=100, minwidth=100, anchor=S)  # 定义列
