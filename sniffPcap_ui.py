...
class GUI:
    ...
    def track_tcp(self):
        ...
        print(itm)
        if not itm:
            tkinter.messagebox.showinfo(title='提示', message='请选择数据包后再追踪流')
            return
    ...
    def check_filter(self,e):
        ...
        try:
            ...
            return
    def button(self):
        self.Button0 = tk.Button(self.frame0, text="Start",bootstyle="dark-outline",command=self.get_packet)
        self.Button0.place(relx=0.85,rely=0.6)
    def interface(self):
        #网卡选项
        var = StringVar()
        ifaces_list=[]
    ...
    def get_packet(self):
        ...
        if self.sniffer:
            ...
            self.sniffer.stop()
            self.sniffer = None
            self.Button0.configure(bootstyle='warning')
        ...
        print(filter_exp)
        if iface is None:
            tkinter.messagebox.showinfo(title='提示', message='请先选择网卡')
            return
        ...
        self.filename = "./pcap{0}.pcap".format(now_time)
        self.sniffer.start()
        self.Button0.configure(bootstyle='dark')
...
#filter = 'tcp.port == 2222'
    ...
    def get_packet_layers(self, packet):
        ...
        while True:
            ...
            counter += 1
    def tree_layer(self):
        self.tree_layer = Treeview(self.frame2,height=9,columns=('qy'),show='tree') 
    ...
    def hex_content(self):
        ...
        self.hex_text = Text(self.frame3, width=120, height=11)
